...
- **Initial transient** is larger than the 2-state filter because the bias loop is spinning up at startup. **Motion does not slow convergence** — the error dynamics are autonomous (the true trajectory cancels out), so bias converges during any flight regime. The transient is purely cold-filter startup, not a stationary-only requirement. Set `bias_gain = 0.0` to recover the 2-state filter if a transient is unacceptable.
- **AHRS attitude error couples into the bias estimate.** During sustained pitch/roll, rotation-induced error in `earth_acceleration().z` is indistinguishable from real accel bias to this filter, so the estimate absorbs it. On a level platform this is zero; on aggressive maneuvers it's a few mg of effective bias. The output is really *bias + averaged rotation-projection error*, not pure sensor bias.
